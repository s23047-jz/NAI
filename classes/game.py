...
class Game(TwoPlayerGame):
    ...
    def _find_four(self):
        for row, col in self.board:
            ...
            while (0 <= row[0] <= 5) and (0 <= row[1] <= 6):
                if self.board[row[0], row[1]] == self._get_current_player_character(self.opponent_index):
                    streak += 1
                    if streak == 4:
                        return True
                else:
    ...
    def make_move(self, column):
        ...
        self.board[row][column] = self._get_current_player_character(self.opponent_index)
